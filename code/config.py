...
class Config:
    ...
    def _load_keyvault_secrets(cls):
        # Only runs if USE_KEY_VAULT is True and KEY_VAULT_URI is set
...
def _initialize_config():
    # Load Key Vault config flags from .env
    USE_KEY_VAULT = os.getenv("USE_KEY_VAULT", "").lower() in ("true", "1", "yes")
    KEY_VAULT_URI = os.getenv("KEY_VAULT_URI", "")
    AZURE_USE_DEFAULT_CREDENTIAL = os.getenv("AZURE_USE_DEFAULT_CREDENTIAL", "").lower() in ("true", "1", "yes")
    ...
    CONFIG_VARIABLES = [
        ...
        # Service Principal (conditionally loaded)
        "AZURE_TENANT_ID",
        "AZURE_CLIENT_ID",
        "AZURE_CLIENT_SECRET",
        # LLM / Model
    ...
    for var_name in CONFIG_VARIABLES:
        ...
        # Azure AI Search vars always from .env
        ...
        # Special: LLM_PROVIDERS and LLM_MODELS can be JSON lists
        if var_name in ("LLM_PROVIDERS", "LLM_MODELS") and value:
            import json
            try:
