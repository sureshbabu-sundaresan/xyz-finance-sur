...
_TEMPERATURE_UNSUPPORTED_MODELS = {
    "gpt-5-mini", "gpt-5-nano", "gpt-5.1-chat",
    "o1-preview", "o1-mini", "o1", "o3-mini",
}


def _resolve_temperature(model: Optional[str], temperature: Optional[float]):
    """Return None if the model does not support temperature."""
    if model and any(model.lower().startswith(m) for m in _TEMPERATURE_UNSUPPORTED_MODELS):
        return None
    return temperature
